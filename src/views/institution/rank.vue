<template>
  <div class="admin-container">
    ...
    <el-row :gutter="40" class="card-group">
      <el-col :xs="96" :sm="96" :lg="48">
        <el-card class="title-card">
          ...
          <div class="form-area">
            <el-table :data="dataList.approveBasicInfoVoList">
    ...
    <el-row v-if="false" :gutter="80" class="pie-chart">
      <el-col :xs="96" :sm="96" :lg="48">
...
export default {
  ...
  data() {
    return {
      ...
      defaultRankName: this.$route.query.ID,
    };
  },

  mounted() {
    this.institutionForm.currentRank = this.getDefaultRankValue();
    this.handleQuery();
  },
  ...
  methods: {
    getDefaultRankValue() {
      for (const option of this.rankOptions) {
        if (option.label === this.defaultRankName) {
          return option.value;
        }
      }
    },
    ...
    async handleQuery() {
      this.institutionForm.currentRank =
        this.institutionForm.currentRank === null
          ? null
          : this.institutionForm.currentRank;
    ...
    async handleSizeChange(size) {
      ...
      if (result.code === 1) {
        ...
        for (const d of this.dataList.approveBasicInfoVoList) {
          d.workJoinDate = this.formatDate(d.workJoinDate);
          d.departmentId = decode.departmentType[d.departmentId];
          d.rankName = decode.rankType[d.rank];
        }
      }
    },
