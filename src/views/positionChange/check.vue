...
<script>
import {reviewDepartmentApplication, getAllDepartmentApplicationList} from "../../apis/positionChange"
...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      const result = await getAllDepartmentApplicationList(this.getTableForm);
    ...
    async approve(item) {
      ...
      const result = await reviewDepartmentApplication(select);
      if (result.code === 1) {
        this.$alert("已批准该申请", {
